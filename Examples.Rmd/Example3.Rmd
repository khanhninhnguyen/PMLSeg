...
We have included a screening function which removes or reduces clusters to single CP based on a significance test comparing the mean before vs. after each cluster. If the change in mean is significant the cluster is replaced by one single CP, otherwise all the CPs in that cluster are removed.
...
To actually remove the data in the time series data frame, do the following:
