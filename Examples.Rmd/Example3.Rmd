...
```{r}
seg_updated <- UpdatedParametersForFixedCP(OneSeries = OneSeries, ResScreening = screening, FunctPart=FALSE, VarMonthly=FALSE)
print(seg_updated)
...
```{r}
PlotSeg(OneSeries = OneSeriesUpd, SegRes = seg_updated, FunctPart = FALSE)
```
...
valid = Validation(OneSeries = df_screened, Tmu = seg_updated$Tmu, MaxDist = 10, Metadata = Metadata)
print(valid)
