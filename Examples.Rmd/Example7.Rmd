...
We see that \code{mBIC} and \code{Lav}, on the one hand, and \code{BM_MJ} and \code{MB_slope}, on the other hand, select the same number of segments \code{K}.
The former two criteria appear slighlty more parsimonious and find the correct number of CPs, while \code{BM_MJ} and \code{MB_slope} have one extra CPs.
Note that the maximum likelihood solutions for a fixed \code{K} are identical, i.e. \code{mBIC} and \code{Lav} give exactly the same solution in terms of \code{seg$Tmu} and \code{seg$CoeffF}, while \code{seg$MonthVar} is unique and does not depend on the criterion.

The spurious CP detected by \code{BM_MJ} and \code{MB_slope} is removed when we apply the selection of functional coefficients \code{selectionF = TRUE} (see Example 6).
